...
export default defineEventHandler(async (event) => {
  try {
    ...
    const customerDetails = {
      first_name: transaction.customer_name || user.value.name || '',
      email: transaction.email || user.value.email || '',
    };
    ...
    await pool.execute(
      `UPDATE transactions 
       SET payment_gateway_payload = JSON_SET(
         ...
         '$.repayment_attempts', COALESCE(JSON_EXTRACT(payment_gateway_payload, '$.repayment_attempts'), 0) + 1,
         '$.last_repayment_time', ?,
         '$.last_repayment_order_id', ?
       )
       WHERE id = ?`,
    ...
    await pool.execute(
      ...
      [
        ...
        transaction.customer_name || user.value.name,
        transaction.email || user.value.email,
        transaction.amount,
