...
export default defineEventHandler(async (event) => {
  try {
    ...
    const deals = rows.map(row => ({
      ...
      price: `Rp${Math.floor(row.new_price).toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`, // For compatibility
      discount: row.discount_percentage ? `${row.discount_percentage}% OFF` : null,
      badge: row.badge,
      backgroundImage: row.background_image_url,
      image: row.background_image_url, // For compatibility
    ...
    // Separate featured and side deals for compatibility
